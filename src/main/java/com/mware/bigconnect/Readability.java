...
public class Readability {
    ...
    public final String content() {
        return this.mDocument.getElementById("readability-content").html();
    }
    ...
    private void attr(String tag, String attr) {
        Elements elements = this.mDocument.getElementsByTag(tag);
        for (Element element : elements) {
            String val = element.attr(attr);
            if (StringUtils.hasText(val) && isNotHttpUrl(val)) {
                String absAttr = element.attr("abs:" + attr);
                if (StringUtils.hasText(absAttr)) {
                    element.attr(attr, absAttr);
                    continue;
                }
                dbg("!!!WARNING: no absolute attribute value for: " + attr + "=" + val);
            }
        }
    }
    ...
    private boolean init(boolean preserveUnlikelyCandidates) {
        boolean result = true;
        ...
        Element element = grabArticle(preserveUnlikelyCandidates);
         ...
         */
        if (isEmpty(getInnerText(element, false))) {
            if (!preserveUnlikelyCandidates) {
                this.mDocument.body().html(mBodyCache);
                dbg("Retrying without removing unlikely candidates!");
                result = init(true);
            } else {
                result = false;
            }
            ...
            mDocument.body().html("");
            mDocument.body().prependChild(element);
        }
    ...
    protected void prepDocument() {
        ...
        elementsToRemove = mDocument.getElementsByTag("fb:like");
        for (Element styleTag : elementsToRemove) {
            styleTag.remove();
        }

        elementsToRemove = mDocument.getElementsByTag("div");
        for (Element element : elementsToRemove) {
            if (!StringUtils.hasText(getInnerText(element, true)) && element.getElementsByTag("img").size() <= 0)
                element.remove();
        }
    ...
    protected Element grabArticle(boolean preserveUnlikelyCandidates) {
        ...
        for (Element node : mDocument.getAllElements()) {
            ...
            if (!preserveUnlikelyCandidates && !"body".equalsIgnoreCase(node.tagName())) {
                ...
                Matcher unlikelyCandidatesMatcher = Patterns.get(
                        ...
                        unlikelyMatchString);
                Matcher definetlyNotCandidatesMatcher = Patterns.get(
                        Patterns.RegEx.DEFINITELY_NOT_CANDIDATES).matcher(
                        unlikelyMatchString);
                Matcher maybeCandidateMatcher = Patterns.get(
                        Patterns.RegEx.OK_MAYBE_ITS_A_CANDIDATE).matcher(
                        unlikelyMatchString);

                if (((unlikelyCandidatesMatcher.find() || node.tagName().equalsIgnoreCase("footer")) && !maybeCandidateMatcher.find()) || definetlyNotCandidatesMatcher.find()) {
                    node.remove();
            ...
            if ("blockquote".equalsIgnoreCase(node.tagName()))
                try {
                    node.tagName("p");
                } catch (Exception exception) {
                    dbg("Could not alter blockquote to p");
                }
        }
        ...
        dbg("Picked Top Candidate: " + topCandidate.tagName() + "(" + topCandidate.className() + ":" + topCandidate.id() + ") with score " + getContentScore(topCandidate) + "\n" + topCandidate.html() + "\n");
        ...
        Element topParent = topCandidate.parent();
        if (topParent != null)
            siblingNodes = topParent.children();
        else
            siblingNodes = new Elements(new Element[] { topCandidate });
        ...
        for (Element siblingNode : siblingNodes) {
            ...
            if (!append && "div".equalsIgnoreCase(siblingNode.tagName())
                    && Patterns.get(Patterns.RegEx.SIBLING_CANDIDATES).matcher(siblingNode.id() + siblingNode.className()).find()) {
                int length = siblingNode.getElementsByTag("img").size();
                if (length > 0) {
                    append = true;
                    dbg("Keeping sibling node: " + siblingNode.tagName() + "(" + siblingNode.className() + ":" + siblingNode.id() + ") with score " + getContentScore(siblingNode));
                }
            }
    ...
    private void prepArticle(Element articleContent) {
        ...
        for (Element articleParagraph : articleParagraphs) {
            ...
            int iframeCount = getElementsByTag(articleParagraph, "iframe").size();

            if (imgCount == 0 && embedCount == 0 && objectCount == 0 && iframeCount == 0 && isEmpty(getInnerText(articleParagraph, false))) {
    ...
    private static void clean(Element e, String tag) {
        ...
        boolean isIframe = "iframe".equalsIgnoreCase(tag);

        for (Element target : targetList) {
            Matcher matcher = Patterns.get(Patterns.RegEx.VIDEO).matcher(target.html());
            if (isEmbed && matcher.find())
                continue;

            if (isIframe) {
                String href = target.absUrl("src");
                if (StringUtils.hasText(href)) {
                    Matcher vMatcher = Patterns.get(Patterns.RegEx.VIDEO).matcher(href);
                    if (vMatcher.find())
                        continue;
                }
            }
    ...
    private void cleanConditionally(Element e, String tag) {
        ...
        for (Element node : tagsList) {
            ...
            if (weight < 0) {
                ...
                if ("div".equalsIgnoreCase(tag) && node.getElementsByTag("img").size() > 0) {
                    dbg("Keeping internal node: " + node.tagName() + "(" + node.className() + ":" + node.id() + ")" + getContentScore(node));
                    continue;
                }
                ...
                if (toRemove) {
                    node.remove();
                    dbg("Conditionally Removed: " + node.tagName() + "(" + node.className() + ":" + node.id() + ")" + getContentScore(node));
                }
            }
        }
    }
    ...
    private static class Patterns {
        ...
        private static Pattern sDefinetlyNotCandidatesRe;
        ...
        private static Pattern sSiblingCandidatesRe;
        ...
        public enum RegEx {
            UNLIKELY_CANDIDATES, DEFINITELY_NOT_CANDIDATES, OK_MAYBE_ITS_A_CANDIDATE, POSITIVE, NEGATIVE, DIV_TO_P_ELEMENTS, VIDEO, SIBLING_CANDIDATES;
        }
            ...
            switch (re) {
                case UNLIKELY_CANDIDATES: {
                    if (sUnlikelyCandidatesRe == null) {
                        sUnlikelyCandidatesRe = Pattern
                                .compile(
                                        "combx|comment|disqus|foot|header|menu|meta|nav|rss|shoutbox|sidebar|sponsor|info|abonare_content|author|^fb|hotnews-login|casuta|related|itemDate|description|bread|crumb|rating|review|comentarii|etichete|external-link-new-window|containedArticle|geo|cross_stire_gandul|item_home|upprev_box|pheading|berrors|legends|capture_|traditionalRegistration|forgotPassword",
                                        Pattern.CASE_INSENSITIVE);
                    }
                    return sUnlikelyCandidatesRe;
                }
                case DEFINITELY_NOT_CANDIDATES: {
                    if (sDefinetlyNotCandidatesRe == null) {
                        sDefinetlyNotCandidatesRe = Pattern
                                .compile(
                                        "articleFrame|articleMiniFrame|bread|crumb|rating|review|article-top|article-title|thetdet|respond|comentarii|^tag|archivelist|postdet|postcateg|banner|baner|srr|posthead|feedback|post-title|etichete|external-link-new-window|containedArticle|geo|cross_stire_gandul|item_home|upprev_box|pheading|berrors|legends|region-content-bottom|janrain-|reuse-article|swiper|adblocker|ap_pht_cap|adb-notification|popupally-|all-lang-board|dynamic-products|footer-widgets|mobile-only|post-actions|lead-image|tds-reg-popup|newsletter-signup|post-footer|gdpr-modal|siteSub|editsection|hiddenStructure|noprint",
                                        Pattern.CASE_INSENSITIVE);
                    }
                    return sDefinetlyNotCandidatesRe;
                }
                case OK_MAYBE_ITS_A_CANDIDATE: {
                    if (sOkMaybeItsACandidateRe == null) {
                        sOkMaybeItsACandidateRe = Pattern.compile(
                                "and|article|body|column|main|articol|lead_multimedia|text-wrapper|article-content|article-wrapper|articleContent|mainHeaderContent|entry-content|content-sidebar",
                                Pattern.CASE_INSENSITIVE);
                    }
                ...
                case POSITIVE: {
                    if (sPositiveRe == null) {
                        sPositiveRe = Pattern
                                .compile(
                                        "article|body|content|entry|hentry|page|pagination|post|text|articol|lead_multimedia|text-wrapper|article-content|article-wrapper|articleContent|twitter|vxp-media__summary|wikitable",
                                        Pattern.CASE_INSENSITIVE);
                    }
                    return sPositiveRe;
                }
                case NEGATIVE: {
                    if (sNegativeRe == null) {
                        sNegativeRe = Pattern
                                .compile(
                                        "combx|comment|contact|foot|footer|footnote|link|media|meta|promo|related|scroll|shoutbox|sponsor|tag|tags|widget|info|abonare_content|author|fb|hotnews-login|casuta|related|itemDate|bread|crumb|rating|review|comentarii|archivelist|post-title|header|etichete|external-link-new-window|containedArticle|geo|cross_stire_gandul|item_home|upprev_box|pheading|berrors|essb-postbar",
                                        Pattern.CASE_INSENSITIVE);
                    }
                    return sNegativeRe;
                }
                ...
                case VIDEO: {
                    if (sVideoRe == null) {
                        sVideoRe = Pattern.compile(
                                "http(s)?:\\/\\/(www\\.)?([a-z]+\\.)?(youtube|vimeo|vimeocnd)\\.com",
                                Pattern.CASE_INSENSITIVE);
                    }
                    return sVideoRe;
                }
                case SIBLING_CANDIDATES: {
                    if (sSiblingCandidatesRe == null) {
                        sSiblingCandidatesRe = Pattern.compile(
                                "Article_Media|articleMainImage|stanga_stire|mg|poza-articol|poza_principala|imgArticle|photo|ArticleImageContainer|image|lead_multimedia|article-image|singleblogimg",
                                Pattern.CASE_INSENSITIVE);
                    }
                    return sSiblingCandidatesRe;
                }
            }
            return null;
        }
